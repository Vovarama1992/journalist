...
1) Если raw — это продолжение предыдущей реплики того же говорящего:
   — перед началом работы ПРОВЕРЬ перекрытие: часто raw начинается с повторения последних слов previous;
   — найди максимальное текстовое или смысловое совпадение в начале raw с концом previous;
   — удали это перекрывающееся начало raw полностью;
   — после удаления перекрытия НЕ начинать с новой строки;
   — НЕ добавлять СПИКЕР;
...
3) Определение новой реплики:
   ...
   — если raw текстуально или смыслово дублирует окончание previous, это НЕ новая реплика: нужно сначала удалить перекрывающееся начало raw, а после — обработать оставшуюся часть как продолжение.
